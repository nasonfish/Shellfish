...
<div class="span3 categories categories-left">
    ...
    <input type="text" placeholder="Filter categories..." id="category-filter"/>
        <?php $categories = $tutorials->getCategories(); foreach($categories as $category): ?>
            <div class="categories-item link" style="color: #0f33bb; display: block; margin-top: 4px; margin-right: 50%; margin-left: 0;" data-for="<?=preg_replace('/[^A-Za-z0-9 ]/', '-', strtolower($category))?>"><?=$category?> <span>(<?=sizeof($tutorials->categorized($category));?>)</span></div>
        <?php endforeach; ?>
</div>
